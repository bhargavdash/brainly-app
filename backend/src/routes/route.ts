...
router.post('/v1/content', authMiddleware,  async(req: CustomerRequest, res: Response): Promise<any> =>{
    try{
...
// get all contents
router.get('/v1/content', authMiddleware, async(req: CustomerRequest, res): Promise<any>=>{
    try{
        const userId = req.userId;
...
// delete a content
router.delete('/v1/content',authMiddleware, async(req: CustomerRequest, res): Promise<any> =>{
    const userId = req.userId;
...
router.post('/v1/brain/share',authMiddleware, async(req: CustomerRequest, res): Promise<any> => {
    const userId = req.userId;
...
router.get('/v1/brain/:shareLink', async(req, res): Promise<any> => {
    try{
        ...
        const getLink = await LinkModel.findOne({hash: shareLink, sharable: true});
        
        if(!getLink){
            return res.status(404).json({error: "Link not found or not accessible"})
        }
        const userId = getLink.userId;
        const contents = await ContentModel.find({userId: userId}).populate('tags');
